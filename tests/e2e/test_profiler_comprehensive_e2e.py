...
@pytest.fixture
def profiler_instance(comprehensive_test_data):
    """Create a fresh Profiler instance for each test method."""
...
def test_profiler_initialization(profiler_instance, comprehensive_test_data) -> None:
    ...
    assert profiler_instance is not None
    assert len(profiler_instance.profiled_columns) == len(dsv_source.columns)

    # Check that profiled columns are copies
    for i, column in enumerate(profiler_instance.profiled_columns):
...
def test_profiler_repr_representation(profiler_instance) -> None:
    ...
    repr_str = repr(profiler_instance)
...
def test_profiler_equality(comprehensive_test_data) -> None:
    ...
    _, _, _, _, data_lake = comprehensive_test_data
    profiler1 = Profiler(data_lake=data_lake)
    profiler2 = Profiler(data_lake=data_lake)
    ...
    profiler3 = Profiler(data_lake=data_lake)  # Fresh profiler with same data lake
...
def test_profiler_comprehensive_profiling(profiler_instance) -> None:
    ...
    profiler_instance.profile(sample_size=500)
...
def test_profiler_sample_size_effectiveness(profiler_instance) -> None:
    ...
    profiler_instance.profile(sample_size=100)
    results_100 = [col.inferred_type for col in profiler_instance.profiled_columns]

    profiler_instance.profile(sample_size=500)
    results_500 = [col.inferred_type for col in profiler_instance.profiled_columns]

    profiler_instance.profile(sample_size=1000)
    results_1000 = [col.inferred_type for col in profiler_instance.profiled_columns]
...
def test_profiler_original_data_unmodified(profiler_instance, comprehensive_test_data) -> None:
    ...
    original_types = [col.inferred_type for col in data_lake.db_source.columns]

    # Run profiling (reduced sample size for performance)
    profiler_instance.profile(sample_size=500)

    # Check that original types are unchanged
    current_types = [col.inferred_type for col in data_lake.db_source.columns]
...
def test_profiler_create_inferred_table(profiler_instance, comprehensive_test_data) -> None:
    ...
    new_table_name = profiler_instance.create_inferred_table()

    # Verify the table was created
    assert new_table_name is not None
    assert new_table_name == f"{data_lake.db_table}_inferred"
    ...
    try:
        with engine.connect() as connection:
            ...
            _verify_casting_examples(connection, new_table_name)
