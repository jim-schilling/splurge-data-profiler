...
Examples:
  ...
  python run_tests.py -x -v all              # Run all tests with fail-fast and verbose output
    ...
    parser.add_argument("--fast", action="store_true", help="Run only fast tests (unit tests)")

    parser.add_argument("--fail-fast", "-x", action="store_true", help="Stop on first failure (pytest -x option)")
    ...
    if args.test_type == "all":
        ...
        # Run core test files
        test_paths = ["tests/test_source.py", "tests/test_data_lake.py", "tests/test_profiler.py", "tests/test_cli.py"]
