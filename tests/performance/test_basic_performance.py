...
def performance_test_setup():
    ...
    dsv_path = test_dir / "performance_test_100k.dsv"
    _generate_dsv(dsv_path, num_rows=100000, delimiter="|", bookend='"')
...
def _run_performance_test(test_dir: Path, dsv_path: Path, num_rows: int) -> Tuple[Dict[str, float], Path]:
    ...
    assert original_count == num_rows, f"Original table should have {num_rows} rows"
...
def test_performance_001k_rows(performance_test_setup) -> None:
    """Test performance with 1,000 rows."""
    test_dir, dsv_path = performance_test_setup
    num_rows = 1000
...
def test_performance_005k_rows(performance_test_setup) -> None:
...
def test_performance_100k_rows(performance_test_setup) -> None:
...
def test_adaptive_sampling_efficiency(performance_test_setup) -> None:
...
def test_operation_breakdown(performance_test_setup) -> None:
    ...
    assert results['profiling_time'] > results['db_creation_time'], \
...
def test_repeated_operations(performance_test_setup) -> None:
